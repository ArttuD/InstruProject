...
model{
  ...
  y ~ normal(mu_type+mu_sample+mu_holder + mu_bead, sigma_common[track_id_ids]);
}
generated quantities {
 ...
 for (i in 1:N) {
    log_likelihood[i] = normal_lpdf(y[i] | mu_type[i]+mu_sample[i]+mu_holder[i]+ mu_bead[i],sigma_common[track_id_ids[i]]);
 } 
 y_hat = normal_rng(mu_type+mu_sample+mu_holder+mu_bead,sigma_common[track_id_ids]);
}